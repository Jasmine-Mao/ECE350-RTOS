...
int main(void) {
	...
	wprintf(
			"Note that a base level of functionality is required in order to run the autograder");
	wprintf(
			"to completion without crashing. Even if you can't get that far right away,");
	wprintf(
			"as you make progress you'll get further and further through the autograder");
	wprintf(
			"code, which can be a great way to gauge your progress in and of itself!");
	wprintf("Cheers and best of luck. Let's get into it! - \x1b[95mJZJ\r\n");

	//Getting into it...
	wprintf(
			"Initializing the kernel and doing some pre-osKernelStart() tests...");

	//Privileged test #1
	if (osTaskExit() != RTX_ERR) {
		rprintf(
				"    osTaskExit() should return RTX_ERR when called from a privileged context!");
	} else {
	...
	if (osKernelStart() != RTX_ERR) {
		rprintf(
				"    osKernelStart() should return RTX_ERR when called before the kernel was initialized!");
	} else {
	...
	if (osGetTID() != 0) {
		rprintf(
				"    osGetTID() should return 0 when called from a privileged context!");
	} else {
	...
	for (task_t ii = 0; ii < MAX_TASKS; ++ii) {
		...
		if (osTaskInfo((task_t) ii, &task_info) != RTX_ERR) {
			rprintf(
					"    osTaskInfo() should return RTX_ERR since no tasks exist yet!");
		...
		if (memcmp(&task_info, &zeroed_task_info, sizeof(TCB)) != 0) {
			rprintf(
					"    osTaskInfo() should not modify the task_info struct when it fails!");
			task_info_passed = false;
			break;
		}
	}
	if (task_info_passed) {
		gprintf(
				"    osTaskInfo() is behaving as expected before the kernel starts!");
		++num_passed;
	}
...
#else
	bprintf(
			"    Skipping k_mem_alloc() test since it's not required for Lab 1...");
	++num_skipped;
#endif
...
#else
	bprintf(
			"    Skipping k_mem_init() test since it's not required for Lab 1...");
	++num_skipped;
#endif
...
#else
	bprintf(
			"    Skipping another k_mem_alloc() test since it's not required for Lab 1...");
	++num_skipped;
#endif
	...
	++num_passed;
	gprintf(
			"Alrighty, the kernel is initialized!\x1b[0m\x1b[1m Let's see how you're doing so far...");
	print_score_so_far();
	...
	for (task_t ii = 0; ii < MAX_TASKS; ++ii) {
		...
		if (osTaskInfo((task_t) ii, &task_info) != RTX_ERR) {
			if (ii == TID_NULL) {    //NULL task special case, see Piazza #101
				if (!task_info.ptask) {
					yprintf(
							"    (osTaskInfo() reporting NULL ptask for the NULL task, maybe this is bad?)");
				}

				if (task_info.stack_size < STACK_SIZE) {
					yprintf(
							"    (osTaskInfo() weird stack size for the NULL task, maybe this is bad?)");
				}
	...
	if (osCreateTask(&test_function_manager_task) == RTX_ERR) { //Corresponds to Lab 1 evaluation outline #1
		rprintf(
				"    osCreateTask() failed to create the test function manager task!");
		rprintf(
				"    Sadly this means we can't really continue, but don't give up! :)");
		while (true)
			;
	} else if (test_function_manager_task.tid == 0) {
		rprintf(
				"    osCreateTask() succeeded but didn't set TID in the task it was passed!");
	} else {
		...
		if (osTaskInfo(ii, &task_info) != RTX_ERR) {
			...
			if (ii == test_function_manager_task.tid) {    //The task we created
				...
				if (task_info.ptask != test_function_manager) {
					rprintf(
							"    osTaskInfo() reporting incorrect ptask, or bad TCB initialization!");
					task_info_passed = false;
					break;
				}
				...
				if (task_info.stack_size < FN_MANAGER_STACK_SIZE) {
					rprintf(
							"    osTaskInfo() reporting incorrect stack size, or bad TCB initialization!");
					task_info_passed = false;
					break;
				}
...
#else
	bprintf(
			"    Skipping another k_mem_init() test since it's not required for Lab 1...");
	++num_skipped;
#endif
	...
	//And off we go!
	wprintf(
			"Okay, I'm calling osKernelStart() now. This is a big step, don't get disheartened");
	wprintf(
			"if it doesn't work on your first try, it certainly didn't for our group :)");
	wprintf("Before we leave though, here's your score so far:");
	print_score_so_far();
	printf("\r\n\r\n");
	osKernelStart();
	assert(
			false && "\x1b[1m\x1b[92mosKernelStart() should never exit when called from a privileged context!\x1b[0m");
}
...
static void print_score_so_far(void) {
	...
	uint32_t jekelscore_whole =
			(total_num == NUM_TESTS) ? 100 : (jekelscore_times_100 / 100);
	uint32_t jekelscore_fraction =
			(total_num == NUM_TESTS) ? 0 : (jekelscore_times_100 % 100);
	wprintf(
			...
			jekelscore_whole, jekelscore_fraction, num_passed, NUM_TESTS,
			num_skipped, NUM_TESTS);
}

static void test_function_manager(void*) {
	gprintf(
			"Haha, awesome you made it! This is being printed from a user task!");
	...
	for (size_t ii = 0; ii < NUM_TEST_FUNCTIONS; ++ii) {
		...
		if (LAB_NUMBER < test_functions[ii].min_labn) {
			bprintf(
					...
					ii + 1, test_functions[ii].name,
					test_functions[ii].min_labn, LAB_NUMBER);
			++num_skipped;
			continue;
		}
		...
		wprintf(
				...
				ii + 1, test_functions[ii].name, test_functions[ii].author,
		...
		if (task.tid == 0) {
			...
			yprintf(
					"You were likely already docked for this so I won't dock you again...\r\n");
		}
	...
	wprintf(
			"Have an idea for a test? Submit a PR at \x1b[96mhttps://github.com/JZJisawesome/autograderv2\x1b[0m\x1b[1m !");
...
static uint32_t mandelbrot_iterations(float creal, float cimag) {
	...
	while ((ii < INSANITY_LEVEL)
			&& (((zreal * zreal) + (zimag * zimag))
					< MANDELBROT_DIVERGE_THRESHOLD)) {
...
static void stack_reuse(void*) { //PARTIALLY corresponds to Lab 1 evaluation outline #10 (less intense, insanity takes care of more)
	...
	tprintf(
			"You passed if those are the same (and both spinners were actually created)!");
...
static void square_batman_helper(void*) {
	...
	for (int ii = 1; ii < 10; ++ii) {
		tprintf("Incrementing counter %d from %d to %d", my_counter,
		...
		if ((ii == 5) && (my_counter == EVIL_ROBIN)) {
			tprintf(
					"I AM EVIL #%d! I'm going to exit early and throw the other Robins off!",
					my_counter);
			osTaskExit();
		}
...
static void square_batman(void*) { //Corresponds to Lab 1 evaluation outline #3 and #4
	...
	while (!all_counters_are_10) {
		...
		if (difference > 1) {
			...
			tprintf(
					"The difference between the highest and lowest Robin counter is %d",
					difference);
			for (int ii = 0; ii < NUM_SIDEKICKS; ++ii) {
...
static void tid_uniqueness(void*) {
	...
	for (int ii = 0; ii < (MAX_TASKS - 3); ++ii) {
		...
		if (tid >= MAX_TASKS) {
			tprintf("TID %d was assigned to a task, which is >= MAX_TASKS!",
					tid);
			treturn(false);
		}
	...
	if (tid_counters[0]) {
		tprintf(
				"One or more of the tasks were assigned TID 0, or the stack allocation failed!");
		treturn(false);
	}
...
static void reincarnation(void*) { //Corresponds to Lab 1 evaluation outline #11
	...
	tprintf(
			"I feel myself slipping away, good thing I'm insured, that's how this works right?");
	osTaskExit();
}
...
static void insanity(void*) { //Corresponds to Lab 1 evaluation outline #10, and also just a really hard test
	...
	tprintf("And goodbye!");
	tprintf(
			"(There would have been a cool mandelbrot stress test here, but there were issues");
	tprintf(
			"I couldn't resolve to due with saving and restoring floating point state.");
	tprintf(
			"So it will be at the end instead, where we don't need to save anything!)");
	treturn(true);
}
...
static void greedy(void*) {    //Corresponds to Lab 1 evaluation outline #9
	...
	tprintf(
			"FOILED AGAIN! I need to work on my money (stack?) laundering skills...");
	tprintf(
			"I've read it's much easier to get away with this if I take in smaller amounts!");
	...
	tprintf(
			"ALL OF YOUR STACK, NO, ALL OF YOUR MEMORY IS MINE! MWAHAHAHAHA!!!!!!");
	tprintf("(Test failed!)");
	treturn(false);
}
